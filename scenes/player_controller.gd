...
@export_range(0.0, 90.0) var max_look_up_degrees := 90.0
...
@onready var debug_box: VBoxContainer = %VBoxContainer
@onready var rayarray = [$ray_container/floor_ray, $ray_container/floor_ray2, $ray_container/floor_ray3, $ray_container/floor_ray4, $ray_container/floor_ray5]
...
var smooth_move_input = Vector3.ZERO
...
var reference_point_prefab := preload("res://scenes/reference_point_for_movement.tscn")
var global_velocity = Vector3.ZERO
var previous_position = Vector3.ZERO

func _ready() -> void:
	Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	floor_ray_length = floor_ray.target_position.y
...
func _process(_delta: float) -> void:
	if delay_counter >= debug_hud_update_delay:
		...
		delay_counter = 0
	delay_counter += 1
	#camera_container.global_position = camera_positioner.global_position - Vector3(0.0, crouch_deepness, 0.0)
...
func _physics_process(delta: float) -> void:
	if Input.is_action_just_pressed("esc"):
		if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
			...
			Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	#rb_top.global_position = rb_base.global_position
	
	global_rotation.y -= (global_rotation.y - camera_container.global_rotation.y)
	player_base.global_position = global_position
	#ray_container.global_position.y = global_position.y - collision_distance
	ray_container.global_position = global_position + Vector3(0.0, height_offset_from_base, 0.0)
...
func handle_debug_hud():
	debug_box.get_node("fps_label").text = "fps:   %s" % Engine.get_frames_per_second()
	debug_box.get_node("state_label").text = "State:   %s" % state.name
	debug_box.get_node("grounded").text = "Grounded:   %s" % is_grounded
	#$DEBUG/VBoxContainer/states.text = "States: %s" % str(states)
	debug_box.get_node("slope_angle_directional").text = "slope_angle_directional:   %s" % snappedf(slope_angle_directional, 0.1)
	debug_box.get_node("slope_angle_global").text = "slope_angle_global:   %s" % snappedf(slope_angle_global, 0.1)
	debug_box.get_node("friction").text = "current_friction:   %s" % snappedf(current_friction, 0.01)
	debug_box.get_node("over_slipping_threshold").text = "is_over_slipping_threshold:   %s" % is_over_slipping_threshold
	debug_box.get_node("current_surface").text = "current_surface:   %s" % current_surface
...
func _integrate_forces(_p_state: PhysicsDirectBodyState3D) -> void:
	...
	if is_grounded:
		...
		if use_shapecast_instead_of_raycast:
			...
			collision_distance = (floor_ray.get_collision_point() - global_position).y
			slope_aligner_helper.global_position = floor_ray.get_collision_point() + Vector3(0.0, 0.01, 0.0)
		...
		if !is_jumping && !is_sliding && !is_over_slipping_threshold:
			...
			apply_central_force(Vector3(0.0, -linear_velocity.y + (10.0 * aligner_y_velocity * abs(deg_to_rad(slope_angle_directional))), 0.0) * 20.0)
		# DAMPENING
	...
	#Move player
	if is_sliding:
		linear_velocity += (move_input.z * global_basis.z + move_input.x * global_basis.x) * current_friction
	else:
		linear_velocity += (move_input.z * slope_aligner.global_basis.z + move_input.x * slope_aligner.global_basis.x) * current_friction
...
func jump(force: float):
	...
	is_jumping = true
	print("JUMPED")
	floor_ray.target_position.y = 0.1
	...
	floor_ray.target_position.y = floor_ray_length
	is_jumping = false
...
func handle_floor_ray():
	...
	if use_shapecast_instead_of_raycast:
		...
		if floor_ray.is_colliding():
			is_grounded = true
			current_surface = floor_ray.get_collider()
		else:
			current_surface = null
...
func handle_crouched_mode():
	if is_crouched:
		...
		tw.tween_property(player_collider, "shape:height", player_original_height / 2.0, 0.5)
		tw.tween_property(player_mesh, "mesh:height", player_original_height / 2.0, 0.5)
		tw.tween_property(self, "crouch_deepness", 0.6, 0.2)
		#player_collider.shape.height = player_original_height / 2.0
		#player_mesh.mesh.height = player_original_height / 2.0
		#crouch_deepness = 0.6
		...
		#player_collider.shape.height = player_original_height
		#player_mesh.mesh.height = player_original_height
		#crouch_deepness = 0.0
